 ...
 ******************
 ...
 FUNCIONES AGREGADORAS
    ...
    EJEMPLO CON BASE DE DATOS COLEGIO

    SELECT YEAR(fechanacimeinto),
    COUNT(id)
    FROM alumnos
    ...
    SELECT SUM (idcurso) AS totalcursos FROM alumnos;
    ...
    SELECT MAX (fechanacimiento) AS alumnoJoven FROM alumnos;
    SELECT MIN(fechanacimiento) AS alumnoJoven FROM alumnos;
 ...
 COMO BUSCAR EN UNA BASE DE DATOS
    ...
    MOSTRAR LOS PROFESORES QUE SON LICENCIADOS Y LICENCIADAS AMBOS

    SELECT * FROM profesores WHERE titulacion LIKE "licenciado%";
    SELECT * FROM profesores WHERE titulacion LIKE "licenciada%";
    SELECT * FROM profesores WHERE titulacion LIKE "licen%";
    SELECT * FROM profesores WHERE titulacion LIKE "%licen%";

    LOS QUE SON ESPECIALISTAS EN PSICOLOGIA Y BIOLOGIA SOLAMENTE

    SELECT * FROM profesores WHERE titulacion LIKE "%PSICOLOGIA";
    SELECT * FROM profesores WHERE titulacion LIKE "%BIOLOGIA";

    LOS PROFESORES CUYO APELLIDO ACABA EN EZ
 ...
 CONCATENAR COLUMNAS (UNIR)
    ...
    EJEMPLOS CON BASE DE DATOS DE COLEGIO

    SELECT CONCAT (nombre,apellidos) AS nombreCompleto FROM profesores;

    SELECT CONCAT (nombre," ", apellidos) AS nombreCompleto FROM profesores;

    SELECT * FROM alumnos WHERE CONCAT(nombre," ",apellidos) LIKE "%yolanda torres%";
 ...
 MUTIPLES CONDICIONES
    ...
    SELECCIONAR LOS ALUMNOS QUE SON DEL 2002 Y QUE SON DEL CURSO CON ID 6

    SELECT *FROM alumnos WHERE YEAR(fechanacimiento)="2002" AND idcurso=10;

    SELECT * FROM alumnos WHERE (idcurso=10 OR idcurso=8 OR idcurso=7) AND YEAR(fechanacimiento)=2002;

    SELECCIONAR LOS ALUMNOS CON ID 10,8,7

    SELECT * FROM alumnos WHERE idcurso IN(10,8,7);

    SELECT * FROM alumnos WHERE idcurso=10 OR idcurso=8 OR idcurso=7;
